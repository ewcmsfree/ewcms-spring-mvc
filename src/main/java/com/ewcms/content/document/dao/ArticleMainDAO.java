...
public class ArticleMainDAO extends JpaDAO<Long, ArticleMain> {
	...
	public Map<Integer, Long> findBeApprovalArticleMain(final String userName,final List<String> groupNames) {
		String hql = "Select new Map(o.channelId, Count(o)) From ArticleMain As o Left Join o.article AS r Left Join r.reviewProcess As p Left Join p.reviewUsers As u Left Join p.reviewGroups As g Where r.delete=false And r.status=:status And (u.userName=:userName ";
		...
		TypedQuery<Map> query = this.getEntityManager().createQuery(hql, Map.class);
		query.setParameter("status", Status.REVIEW);
		query.setParameter("userName", userName);
		
		return (Map<Integer, Long>) query.getResultList();
	}
